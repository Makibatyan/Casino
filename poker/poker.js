...
playerCall() {
    ...
    if (callAmount <= 0) {
        this.updateGameStatus('コールする必要はありません。');
        return;
    }
    ...
    this.playerBet += actualCallAmount; // そのラウンドの累計支出を更新
    
    // 2. ポットへの追加（代入ではなく加算）
    this.pot += actualCallAmount;
    
    // 3. ショートコール（オールイン）時の差額調整
    if (actualCallAmount < callAmount) {
        const surplus = callAmount - actualCallAmount;
        
        // CPUに差額を返却
        this.computerChips += surplus;
        // ポットからも差額を引く（CPUが既にポットに入れていた分を戻す）
        this.pot -= surplus;
        
        // 場の最大ベット額とCPUのベット額をプレイヤーの限界値に合わせる
        this.currentBet = this.playerBet;
        this.computerBet = this.playerBet; 
        
        this.updateGameStatus(`${actualCallAmount}でオールイン！CPUの過剰分${surplus}を返却しました。`);
    } else {
        if (this.playerChips === 0) {
            this.updateGameStatus(`${actualCallAmount}でピッタリ！オールインしました。`);
        } else {
            this.updateGameStatus(`${actualCallAmount}チップコールしました。`);
        }
    }
    ...
    allPlayersActed() {
        ...
        // プレイヤーがフォールドしていない場合
        if (this.playerBet >= 0) {
            activeBets.push(this.playerBet);
        }
        // コンピューターがフォールドしていない場合
        if (this.computerBet >= 0) {
            activeBets.push(this.computerBet);
        }
        
        // フォールドしたプレイヤーを除外して、残りのプレイヤー全員がベット揃いしているかチェック
        if (activeBets.length === 0) return false;
        
        // 残っているプレイヤー全員が現在のベット額に揃っているか
